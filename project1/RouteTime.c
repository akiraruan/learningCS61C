...
/*
 *  Given a file with routes and a flight system, parses the file and determine
 * (and print) the earliest time each route can be completed. Lines of bad form
 * are skipped while new lines are ignored.
 */
void calcRouteTimes(flightSys_t *s, FILE *routes) {
    ...
    /*
     *  This while loop parses the file line by line and calculate the route
     * times as it goes. If a new route line is found, the current route (if it
     * exists) would be done and the result is printed. A current route exists
     * if curAirport is not null.
     *
     *  curAirport tells us where we are on the current route.
     *
     *  now tells us the current time (the earliest time we can reach the
     * curAirport on the current route).
     */
    while (fgets(line, MAX_LINE_LEN, routes)) {
        ...
        if (!strlen(line)) {
            continue; // ignore line if it's empty
                           ...
                           prefixLen) ==
                   0) { // if beginning of line starts with "ROUTE: ", we are
                        // calculating for a new route
            if (curAirport) {
                printCompleteRoute(
                    route, &now,
                    totalCost); // if curAirport is not NULL, we are done with
                                // the previous route so print the result
            }
            curAirport = NULL;
            totalCost = 0;
            if (3 != sscanf(line + prefixLen, "%s %s %s", route, airportBuf,
                            timeBuf) // parse rest of new route line into route
                                     // name, airport name, and start time
                || !stringToTime(timeBuf, &now) // ensure time is valid
                ||
                !(curAirport = getAirport(
                      s, airportBuf))) { // ensure airport exists and sets
                                         // curAirport to the starting airport
                printf("Skipping line: %s\n", line);
                continue;
            }
        } else if (curAirport) {
            /*
             *  This is the case when we are in the middle of calculating for a
             * route, and the line should just be next airport we have to get
             * to. Here, we use getNextFlight to determine the flight to take to
             * the next airport. If there are no possible flights, the route
             * cannot be completed.
             */
            ...
            timeHM_t depart;
            timeHM_t arrival;
            if (!getNextFlight(curAirport, next, &now, &depart, &arrival,
                               &curCost)) {
                curAirport = NULL;
                totalCost = 0;
...
/*
 *  Given a flight system with airports, and a file of schedules for those
 * airports, add flight times to the system.
 */
void parseSchedule(flightSys_t *s, FILE *schedule) {
     ...
     *
     *  The while loop parses the file line by line and sets a new curAirport
     * when a line that starts with "STATION: " is found. It'll also echo the
     * schedule of the previous airport. Otherwise, it treats the current line
     * as a new entry in the schedule of the curAirport and calls addFlight.
     */
    char line[MAX_LINE_LEN];
    while (fgets(line, MAX_LINE_LEN, schedule)) {
        if (!strcmp(line, "\n") || !strcmp(line, "\r\n")) {
            ...
            if (curAirport) {
                printSchedule(
                    curAirport); // done with previous schedule so print it
            }
            stripNewLine(line);
            ...
            if (4 !=
                sscanf(
                    line, "%s %s %s $%s", dstName, departureStr, arrivalStr,
                    priceStr)) { // parses the line as destination airport name,
            ...
            if (!stringToTime(arrivalStr, &arrival) ||
                !stringToTime(departureStr, &departure) || *endptr) {
                printf("Skipping line: %s\n", line);
                continue;
            }
...
/*
 *  Calls parseAirports, parseSchedules, and calcRouteTimes if the required
 * files exists.
 */
...
void validateFlightPathTestStudent() {
    ...
    flight_t *f1 = createFlight(SFO, &(timeHM_t){.hours = 12, .minutes = 0},
                                &(timeHM_t){.hours = 13, .minutes = 0}, 200),
             *f2 = createFlight(SEA, &(timeHM_t){.hours = 13, .minutes = 0},
                                &(timeHM_t){.hours = 14, .minutes = 0}, 300);
    char *temp1[] = {"SFO", "SEA"};
    flight_t *fl1[] = {f1, f2};
...
int main(int argc, char *args[]) {
    ...
    if (false) {
        ...
        printf("Given flight schedules and a route with a starting time, this "
               "program will determine when the route will be completed.\n");
        printf("This configuration should be able to support tests with many "
               "different flight systems.\n");
        printf("Each line of the config file should be <file of airport "
               "names>, <file of schedules>, <file of routes>.\n");
        printf("\t<file of airport names> should contain a list of airport "
               "names, one on each line.\n");
        printf("\t<file of schedules> should contain a list of schedules. \n");
        printf("\t\t\"AIRPORT: <airport name>\" marks the beginning of a "
               "schedule for that airport.\n");
        printf("\t\tSubsequent lines contain the destination airport, "
               "departure time, and arrival time separated by spaces\n");
        printf("\t<file of routes> should contain a list of routes. \n");
        printf("\t\t\"ROUTE: <route name> <starting airport> <begin time>\" "
               "marks the beginning of a plan starting at the specified "
               "airport and time.\n");
        printf("\t\tSubsequent lines contain the airports in the route, one on "
               "each line, in order\n");
        printf("Each line in the config file will create a separate flight "
               "system and will assume no dependencies with previous lines.\n");
        return 0;
    }
    char *a = "integration_config";
    configFile = fopen(a, read);
    if (!configFile) {
